...
export default {
  createItem: async (newItem: ProductGroup, accessToken: string): Promise<ResponseDataType> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItemByPk: async (id: number, itemToUpdate: ProductGroup, accessToken: string): Promise<ResponseDataType> => {
    return client
