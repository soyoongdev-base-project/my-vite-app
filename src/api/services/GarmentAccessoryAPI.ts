...
export default {
  createItem: async (newItem: GarmentAccessory, accessToken: string): Promise<ResponseDataType> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItems: async (itemsToUpdate: GarmentAccessory[], accessToken: string): Promise<ResponseDataType> => {
    return client
