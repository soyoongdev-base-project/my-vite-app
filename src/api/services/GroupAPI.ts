...
export default {
  createItem: async (newItem: Group, accessToken: string): Promise<ResponseDataType> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItemByPk: async (id: number, itemToUpdate: Group, accessToken: string): Promise<ResponseDataType> => {
    return client
