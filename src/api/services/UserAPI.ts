...
export default {
  createItem: async (newItem: User, accessToken: string): Promise<ResponseDataType> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItemByPk: async (id: number, itemToUpdate: User, accessToken: string): Promise<ResponseDataType> => {
    return client
